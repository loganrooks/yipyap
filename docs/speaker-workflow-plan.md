...
1. Validation: every cluster must have one resolved disposition —
   ...
   constraints:
   - `merge_with` targets must themselves resolve transitively to a
     `label` or `split` disposition (not another `merge_with` or
     `ignore`). Cycles are an error.
...
the artifact and triggers re-ingest. `separation.{voice,background}_path`
is recorded explicitly so render can mix against the same background
stem that ingest produced, even if the working directory layout
changes between stages.

### `clip.speakers.json`

Two states. Both states bind back to the ingest artifact via
`source_sha256` (the same value as `clip.ingest.json`'s
`source.sha256`); if the clip is re-ingested with different content,
the sidecar fingerprint mismatch invalidates the sidecar before any
labels are applied.
...
```json
{
  "profile_id": "01HF3K8Z4Y9N2P5E7QABCDEF12",
  "name": "Crofty",
  "name_history": [
